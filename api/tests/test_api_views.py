...
from manager.models import Account, Character, Equipment
from manager.serializers.account import AccountSerializer, CreateAccountSerializer
from manager.serializers.character import CharacterSerializer, CreateCharacterSerializer
...
class TestAPIViews(TestCase):
    ...
    def test_is_authenticated(self):
        self._require_login()
        response = self.client.get(f'/api/characters/{self.char.name}/')
    ...
    def test_is_char_owner_permission(self):
        ...
        response = self.client.get(f'/api/characters/{self.char.name}/equipment/')
        self.assertEqual(response.status_code, HTTP_404_NOT_FOUND)

    def test_get_equipment_char_not_exists(self):
        self._require_login()
        test_charname = 'charthatdoesnotexist'
        response = self.client.get(f'/api/characters/{test_charname}/equipment/')
        self.assertEqual(response.status_code, HTTP_404_NOT_FOUND)
    ...
    def test_get_equipment_which_does_not_exist(self):
        ...
        self._require_login()
        response = self.client.get(f'/api/characters/{char_wout_eq.name}/equipment/')
        self.assertEqual(response.status_code, HTTP_404_NOT_FOUND)
    ...
    def test_update_equipment_but_char_does_not_exist(self):
        ...
        response = self.client.post(
            f'/api/characters/{test_charname}/equipment/', data={'a': 'b'})
        self.assertEqual(response.status_code, HTTP_404_NOT_FOUND)
    ...
    def test_update_equipment_but_it_does_not_exist(self):
        ...
        Equipment.objects.get(char=char_wout_eq).delete()
        response = self.client.post(
            f'/api/characters/{char_wout_eq.name}/equipment/', data={'random': 'data'}
        )
        self.assertEqual(response.status_code, HTTP_404_NOT_FOUND)
    ...
    def test_update_equipment_single_field(self):
        self._require_login()
        response = self.client.post(
            f'/api/characters/{self.char.name}/equipment/',
    ...
    def test_list_characters(self):
        ...
        self.assertEqual(
            response.json(),
            CharacterSerializer(Character.objects.filter(acc__profile__user=self.user), many=True).data
        )
    ...
    def test_update_character_char_doesnt_exist(self):
        self._require_login()
        response = self.client.patch('/api/characters/XDDDDDD/', {'name': 'wont happen'})
        self.assertEqual(response.status_code, HTTP_404_NOT_FOUND)

    def test_update_character_by_not_owner(self):
        self._require_login()
        response = self.client.patch(f'/api/characters/{self.other_user_char.name}/', {'level': 55})
        self.assertEqual(response.status_code, HTTP_404_NOT_FOUND)
    ...
    def test_update_character(self):
        ...
        response = self.client.put(f'/api/characters/{self.char2.name}/', {
            'level': 92,
            'expansion': False,
            'char_class': 'Druid',
        })
    ...
    def test_update_character_partially(self):
        ...
        response = self.client.patch(f'/api/characters/{self.char.name}/', {
            'level': new_level,
        })
    ...
    def test_delete_char_by_not_owner(self):
        self._require_login()
        response = self.client.delete(f'/api/characters/{self.other_user_char.name}/')
        self.assertEqual(response.status_code, HTTP_404_NOT_FOUND)

    def test_delete_character(self):
        self._require_login()
        response = self.client.delete(f'/api/characters/{self.char2.name}/')
    ...
    def test_list_accounts(self):
        ...
        self.assertEqual(
            response.json(),
            AccountSerializer(Account.objects.filter(profile__user=self.user), many=True).data
        )
    ...
    def test_update_account(self):
        ...
        response = self.client.patch(f'/api/accounts/{self.acc.name}/', {
            'realm': 'US East',
        })
        self.assertEqual(response.status_code, HTTP_200_OK)
    ...
    def test_update_account_invalid_data(self):
        self._require_login()
        response = self.client.put(f'/api/accounts/{self.acc.name}/', {
            'realm': 'lmao',
        })
        self.assertEqual(response.status_code, HTTP_400_BAD_REQUEST)
        self.assertEqual(
    ...
    def test_update_account_by_not_owner(self):
        self._require_login()
        response = self.client.patch(f'/api/accounts/{self.acc2.name}/', {
            'realm': 'US West'
        })
        self.assertEqual(response.status_code, HTTP_404_NOT_FOUND)
    ...
    def test_delete_account(self):
        ...
        response = self.client.delete(f'/api/accounts/{lookup_name}/')
        self.assertEqual(response.status_code, HTTP_204_NO_CONTENT)
